 ...
 * @usage execVM "core\scripts\R3F\LOG\addons_config\logistics_config_maker_tool\launch_config_tool.sqf";
 *
...
player addAction ["Dump config to RPT", "core\scripts\R3F\LOG\addons_config\logistics_config_maker_tool\dump_config.sqf"];
...
while {BOOL_continue} do
{
	...
	while {_idx_current_vehicle == IDX_requested_vehicle} do
	{
		...
		_class_infos = _class_infos + "<t color='#FFFFFF'>Specific config :</t><br/>";
		_class_infos = _class_infos + format ["<t color='#FFFFFF'>[%1] Movable</t><br/>", if (_class_name in R3F_LOG_CFG_can_be_moved_by_player) then {"M"} else {" "}];
		_class_infos = _class_infos + format ["<t color='#FFFFFF'>[%1] Liftable</t><br/>", if (_class_name in R3F_LOG_CFG_can_be_lifted) then {"L"} else {" "}];
		_class_infos = _class_infos + format ["<t color='#FFFFFF'>[%1] Towable</t><br/>", if (_class_name in R3F_LOG_CFG_can_be_towed) then {"T"} else {" "}];
		_class_infos = _class_infos + format ["<t color='#FFFFFF'>[%1] Cargo-able (transport)</t><br/>", if (_class_name in R3F_LOG_classes_objets_transportables) then {format ["C%1", R3F_LOG_CFG_can_be_transported_cargo select (R3F_LOG_classes_objets_transportables find _class_name) select 1]} else {" "}];
		_class_infos = _class_infos + "<br/>";
		_class_infos = _class_infos + format ["<t color='#FFFFFF'>[%1] Lifter</t><br/>", if (_class_name in R3F_LOG_CFG_can_lift) then {"L"} else {" "}];
		_class_infos = _class_infos + format ["<t color='#FFFFFF'>[%1] Tower</t><br/>", if (_class_name in R3F_LOG_CFG_can_tow) then {"T"} else {" "}];
		_class_infos = _class_infos + format ["<t color='#FFFFFF'>[%1] Cargo (transporter)</t><br/>", if (_class_name in R3F_LOG_classes_transporteurs) then {format ["C%1", R3F_LOG_CFG_can_transport_cargo select (R3F_LOG_classes_transporteurs find _class_name) select 1]} else {" "}];
		_class_infos = _class_infos + "<br/>";
		_class_infos = _class_infos + "<t color='#FFFFFF'>Inherited config :</t><br/>";
		...
		for [{_j = 0}, {_j < count _tab_inheritance_tree}, {_j = _j+1}] do
		{
			...
			then {_options = _options + "<t color='#FFFFFF'>M</t>";};

			if (_class_name_inherit in R3F_LOG_CFG_can_be_lifted)
			then {_options = _options + "<t color='#FFFFFF'>L</t>";};

			if (_class_name_inherit in R3F_LOG_CFG_can_be_towed)
			then {_options = _options + "<t color='#FFFFFF'>T</t>";};

			if (_class_name_inherit in R3F_LOG_classes_objets_transportables)
			then {_options = _options + format ["<t color='#FFFFFF'>C%1</t>", R3F_LOG_CFG_can_be_transported_cargo select (R3F_LOG_classes_objets_transportables find _class_name_inherit) select 1];};
			...
			then {_options = _options + format ["<t color='#FFFFFF'>C%1</t>", R3F_LOG_CFG_can_transport_cargo select (R3F_LOG_classes_transporteurs find _class_name_inherit) select 1];};
