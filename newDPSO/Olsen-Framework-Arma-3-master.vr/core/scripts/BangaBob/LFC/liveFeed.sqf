...
private ["_name","_cam","_debug","_class","_newClass","_arguments"];
	...
	if (!((headgear _unit) in DPSO_friendly_cameraHelmets))
...
while {true} do {
	...
	if (!((headgear _unit) in DPSO_friendly_cameraHelmets)) exitWith
		{
			detach _cam;
	...
	then {_monitor setObjectTextureglobal [0,"scripts\BangaBob\LFC\failed.paa"]; sleep 3; _monitor setObjectTexture [0,'#(argb,8,8,3)color(0,0,1,1)']} else
	{
		...
		if (isnil "_cam")
			...
			if (!("ItemGPS" in assignedItems _unit)) exitWith
				...
				if (DPSO_sunHeight > 0) then {[_monitor,_name,_cam,0] remoteExec ["fnc_RENDER"]} else {[_monitor,_name,_cam,2] remoteExec ["fnc_RENDER"]};
};
};
