...
if (_unit getVariable ["ais_stabilized", false]) exitWith {
	...
	titleText [localize "STR_AIS_stabilized", "PLAIN DOWN", 1];
...
if (AIS_SHOW_COUNTDOWN) then {
	[titleText [localize "STR_AIS_playerDead","PLAIN DOWN",1], titleText [format [localize "STR_AIS_playerBleeding", floor _timeleft],"PLAIN DOWN",0.2]] select (_timeleft > 3);
};

_unit setFatigue 1;
hintSilent localize "STR_hint_AIS_pressH";
