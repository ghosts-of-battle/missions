...
if (_type == "stabilize") then
	{
		_kit_pos = _healer modelToWorld [-0.5,0.2,0];
		_kit = "Land_Antibiotic_F" createVehicle _kit_pos;
		[_kit, [_kit_pos select 0, _kit_pos select 1, 0]] call AIS_Core_fnc_setPosAGLS;
		_kit setDir (random 359);
		ais_medequip_array pushBack [0,_kit];
		...
		//_healer removeItem OPEX_medical_firstAidKit;
	};
...
for "_i" from 1 to (1 + (round random 3)) do
{
	...
	[_band, [_band_pos select 0, _band_pos select 1, 0]] call AIS_Core_fnc_setPosAGLS;
	_band setDir (random 359);
	if (_i > 1) then {ais_medequip_array pushBack [0,_band]} else {ais_medequip_array pushBack [1,_band]};
};
