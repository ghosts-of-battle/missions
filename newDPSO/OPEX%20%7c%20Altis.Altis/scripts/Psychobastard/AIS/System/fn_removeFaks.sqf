...
{
	if (_x == OPEX_medical_firstAidKit) then {
		_numFakUniform = _numFakUniform + 1;
		_unit removeItemFromUniform OPEX_medical_firstAidKit;
	};
	nil
...
// Faks from the vest
{
	if (_x == OPEX_medical_firstAidKit) then {
		...
		_unit removeItemFromVest OPEX_medical_firstAidKit;
	};
	nil
...
// Faks and Medikits from the backpack. Kits can only be in backpack, so we don't search for them anywhere else
{
	if (_x == OPEX_medical_firstAidKit) then {
		...
		_unit removeItemFromBackpack OPEX_medical_firstAidKit;
	};
	if (_x == "Medikit") then {
		_numMedi = _numMedi + 1;
