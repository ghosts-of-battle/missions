...
_natural = (sunOrMoon - _clouds) + (0.3 * moonIntensity);
_artificial = [0, [0,0,0]];
...
if (_natural < 0.5) then {
	...
	//lamps/lights
	_allLamps = nearestObjects [player, ["Lamps_base_F", "Land_PowerPoleWooden_L_F", "TentLamp_01_base_F"], 60];
	...
	//fires
	_allFires = nearestObjects [player, ["Land_FirePlace_F", "Land_FirePlace_F", "MetalBarrel_burning_F"], 20];
	if (count _allFires > 0) then {
		{
			...
			if (inflamed _fire) then {
				...
				_fireLight = [1 - ((_playerPos distance _pos) / 15), _colour];
			};
	...
	if (count _allFlashlights > 0) then {
		{
			...
			if (_man isFlashlightOn currentWeapon _man) then {
				...
				if (_diff < 0.5) then {
					_item = if ((currentWeapon _man) == primaryWeapon _man) then {(primaryWeaponItems _man) select 1} else {(handGunItems _man) select 1};
	...
	// logics
	_logicLights = (nearestObjects [player, [], 25]) select {typeOf _x == "#lightpoint"};
	if (count _logicLights > 0) then {
		{
			_logic = _x;
			_playerPos = player modelToWorld (player selectionPosition "spine3");
			_fullDistance = 10;
			_colour = [1,1,1];
			_logicLight = [1 - ((_playerPos distance2D _x) / _fullDistance), _colour];
			//_logicLight = [(1 - ((_playerPos distance _startPos) / _fullDistance)) - _diff / 8, _colour];
			if (count _logicLight > 1) exitWith {};
		} forEach _logicLights;
	};
	...
	} forEach [_lampLight, _carLight, _fireLight, _chemLight, _flashLight, _logicLight];
	...
	if (count _nearestLight > 1) then {
		...
		//if (_intensity < 0) then {_intensity = 0} else {_intensity = _intensity + 0.15};
		if ((_intensity > 0.01) && (_intensity < 0.60)) then {_intensity = _intensity * (1 + (0.75 - _intensity))};
		if (_intensity < 0.01) then {_intensity = 0.01} else {if (_intensity > 1) then {_intensity = 1}};
		_artificial = [_natural + _intensity, _colour];
	} else {_artificial = [0.05,[0,0,0]]};
