...
#define HAS_LIGHT (({(isNumber (configFile >> "CfgWeapons" >> _x >> "ItemInfo" >> "Flashlight" >> "Intensity"))} count (items player) > 0) || (isNumber (configFile >> "CfgWeapons" >> (primaryWeaponItems player select 1) >> "ItemInfo" >> "Flashlight" >> "Intensity")))
...
if (isNumber (configFile >> "CfgWorlds" >> worldName >> "mapSize")) then {_worldSize = getNumber (configFile >> "CfgWorlds" >> worldName >> "mapSize")} else
	{
		...
		/*
		_worldSize = switch (worldName) do
		{
...
vip_lit_var_cl_mapRainTypes = [
	_missionRoot + "scripts\Viperidae\vip_lit\resource\drop1.paa",
	_missionRoot + "scripts\Viperidae\vip_lit\resource\drop2.paa",
	_missionRoot + "scripts\Viperidae\vip_lit\resource\drop3.paa",
	_missionRoot + "scripts\Viperidae\vip_lit\resource\drop4.paa",
	_missionRoot + "scripts\Viperidae\vip_lit\resource\drop5.paa",
	_missionRoot + "scripts\Viperidae\vip_lit\resource\drop6.paa",
	_missionRoot + "scripts\Viperidae\vip_lit\resource\drop7.paa"
];
...
vip_lit_var_cl_mapFlashlightTex = _missionRoot + "scripts\Viperidae\vip_lit\resource\flashlight.paa";
...
vip_lit_eh_cl_drawFlashlight = (findDisplay 12 displayCtrl 51) ctrlAddEventHandler ["Draw", {
	...
	_sizeBeam = _size / vip_lit_var_cl_mapFlashlightScale / 2; if (_mapZoom < 0.2) then {_sizeBeam = _sizeBeam / (_mapZoom * (1 / 0.2))};
...
/*
// disabled by Gemini because it causes an error:
// https://forums.bohemia.net/forums/topic/179455-little-immersion-tweaks-spice-up-arma-3s-night-life/?do=findComment&comment=3295345
