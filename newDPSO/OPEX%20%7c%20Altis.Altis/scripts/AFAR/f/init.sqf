...
if (!isDedicated) then {waitUntil {!isNil "OPEX_introFinished"}; waitUntil {OPEX_introFinished}; sleep 15};

if(actionKeys"diary"isEqualTo[])then{systemChat localize "STR_afar_warning1"};
if(actionKeys"PushToTalk"isEqualTo[])then{systemChat localize "STR_afar_warning2"};
if(!(actionKeys"VoiceOverNet"isEqualTo[]))then{systemChat localize "STR_afar_warning3"};
