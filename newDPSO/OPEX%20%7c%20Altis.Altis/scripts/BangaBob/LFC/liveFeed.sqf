		...
		if (isnil "_cameraCount")
			...
			_cameraCount = 0;server setvariable ["CAMCOUNT",_cameraCount,true];
...

				{
					...
					_uniqueName=format ["<t color=""#FFFFFF"">" + localize "STR_action_liveFeedHelmet" + "</t>",name _x];
					liveFeedAction = _monitor addAction [_uniqueName, "_this remoteExec ['fnc_PROCESS_LIVE']",[_x,_debug],6,false,true,"","_target distance _this < 3.5"];

					_uniqueName=format ["<t color=""#FFFFFF"">" + localize "STR_action_liveFeedSat" + "</t>",name _x];
					_monitor addAction [_uniqueName, "_this remoteExec ['fnc_PROCESS_SATT']",[_x,_debug],6,false,true,"","_target distance _this < 3.5"];

					_uniqueName=format ["<t color=""#FFFFFF"">" + localize "STR_action_liveFeedClose" + "</t>",name _x];
					_monitor addAction [_uniqueName, "_this remoteExec ['fnc_SHUTDOWN_MONITOR']",[_x,_debug],6,false,true,"","_target distance _this < 3.5"];

					_x setvariable ["LIVEid",_cameraCount,true];};};
				} foreach _units;


	if (_debug) then {_sidechatHint = format["Total Cam count: %1", _cameraCount];player sideChat _sidechatHint;GlobalSideChat = _sidechatHint; publicVariable "GlobalSideChat";};


fnc_SHUTDOWN_MONITOR =
{
	private ["_monitor", "_arguments", "_unit", "_cam"];
	_monitor = (_this select 0);
	_arguments = (_this select 3);
	_unit = _arguments select 0;
	_cam = _unit getVariable "LIVECAM";

	_monitor setObjectTexture [0,'#(argb,8,8,3)color(0,0,0,1)'];
...
private ["_name","_cam","_debug","_class","_newClass","_arguments"];
	...
	_monitor setObjectTextureglobal [0,"scripts\BangaBob\LFC\connecting.paa"];
	SLEEP (2 + random 5);

	if (!((headgear _unit) in OPEX_friendly_cameraHelmets))
	then {_monitor setObjectTextureglobal [0,"scripts\BangaBob\LFC\failed.paa"]; sleep 3; _monitor setObjectTexture [0,'#(argb,8,8,3)color(0,0,1,1)']} else
	{
	...
	_cam=_unit getVariable "LIVECAM";
				...
				if (isnil "_cam")
						...
						_class = typeOf (vehicle _unit);
							...
							case "B_T_Truck_01_box_F": {_cam attachto [(vehicle _unit),[-0.3,3.2,0.5], "neck"];};
							...
							case "C_Van_01_fuel_F": {_cam attachto [(vehicle _unit),[-0.4,0.4,0.3], "neck"];};
							   ...
							   default
							{
							_cam attachto [_unit,[0,0.20,0.20], "neck"];
								};
							};
		...
		_name=format ["rendertarget%1",_id];
			...
			[_monitor,_name,_cam,0] remoteExec ["fnc_RENDER"];
...
while {true} do {
	...
	_NeWclass = typeOf (vehicle _unit);
			...
			if (!(_NeWclass == _class))
				...
				[_monitor,0,0,_arguments] remoteExec ["fnc_PROCESS_LIVE"];
				};

	if (!((headgear _unit) in OPEX_friendly_cameraHelmets)) exitWith
		{
			detach _cam;
			_cam cameraEffect ["TERMINATE", "BACK"];
			camDestroy _cam;
			_monitor setObjectTextureglobal [0,"scripts\BangaBob\LFC\failed.paa"];
			sleep 3;
...
private ["_name","_cam","_debug"];
	...
	then {_monitor setObjectTextureglobal [0,"scripts\BangaBob\LFC\failed.paa"]; sleep 3; _monitor setObjectTexture [0,'#(argb,8,8,3)color(0,0,1,1)']} else
	{
		...
		if (isnil "_cam")
				then {
					_cam = "camera" camCreate [(getpos _unit select 0), (getpos _unit select 1), (getpos _unit select 2) + 300];
						_unit setvariable ["SATTCAM",_cam,true];
							...
							if (isnull _cam)
								then {
									_cam = "camera" camCreate [(getpos _unit select 0), (getpos _unit select 1), (getpos _unit select 2) + 250];
									_unit setvariable ["SATTCAM",_cam,true];
									//0=[_cam,_debug] call fnc_RESET;
									[_cam,_debug] remoteExec ["fnc_RESET"];
										};
			...
			_cam setpos [(getpos _unit select 0), (getpos _unit select 1), (getpos _unit select 2) + 250];
							sleep 1;
			if (!("ItemGPS" in assignedItems _unit)) exitWith
				...
				if (OPEX_sunHeight > 0) then {[_monitor,_name,_cam,0] remoteExec ["fnc_RENDER"]} else {[_monitor,_name,_cam,2] remoteExec ["fnc_RENDER"]};
};
};
...
if (_debug) then {_sidechatHint = format["Concurrent cam Count: %1",count _conCurrentCams];PLAYER sideChat _sidechatHint;GlobalSideChat = _sidechatHint; publicVariable "GlobalSideChat";};
...
if (_debug) then {_sidechatHint = format["Deleted CONCURRENTCAMS: %1",count _conCurrentCams];PLAYER sideChat _sidechatHint;GlobalSideChat = _sidechatHint; publicVariable "GlobalSideChat";};
					};
			};
