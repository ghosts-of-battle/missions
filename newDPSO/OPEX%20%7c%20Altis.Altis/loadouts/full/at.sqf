...
// ----------------------------------------------------------------------
	...
	// VEST
	_unit addVest (selectRandom (OPEX_friendly_commonVests + OPEX_friendly_EODvests));
...
// ----------------------------------------------------------------------
	...
	// LAUNCHER
	_launcher = selectRandom OPEX_friendly_ATlaunchers;
	[_unit, _launcher] call Gemini_fnc_addLoadedWeapon;

	// ADDITIONAL MAGAZINES
	_launcherMagazine = (getArray (configfile >> "CfgWeapons" >> _launcher >> "magazines")) select 0;
	for "_i" from 1 to 1 do {_unit addItemToBackpack _launcherMagazine};
