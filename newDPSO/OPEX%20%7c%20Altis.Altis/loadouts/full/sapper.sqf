...
// ----------------------------------------------------------------------
	...
	// VEST
	_unit addVest (selectRandom OPEX_friendly_EODvests);

	// HEADGEAR
	_unit addHeadgear (selectRandom OPEX_friendly_EODhelmets);
...
// ----------------------------------------------------------------------
	...
	// NVG
	if (random 1 > 0.5) then {_unit linkItem (selectRandom OPEX_friendly_NVGs)} else {_unit addItemToVest (selectRandom OPEX_friendly_NVGs)};
...
// ----------------------------------------------------------------------
	...
	for "_i" from 1 to 1 do {_unit addItemToBackpack (selectRandom OPEX_friendly_toolKits)};
	if (isClass (configFile >> "CfgPatches" >> "ace_explosives")) then {for "_i" from 1 to 1 do {_unit addItemToBackpack "ACE_DefusalKit"}};
	if (isClass (configFile >> "CfgPatches" >> "ace_explosives")) then {for "_i" from 1 to 1 do {_unit addItemToBackpack "ACE_Clacker"}};
	private ["_index"]; for "_i" from 1 to 1 do {_index = 0; for "_index" from 0 to 5 do {_unit addItemToBackpack (OPEX_friendly_explosives select _index); _index = _index + 1}};
