...
// =========================================================================================================
	...
	[DPSO_taskID, "interact", "taskMarker_A"] remoteExec ["Gemini_fnc_taskAssigned"];
...
// =========================================================================================================
	...
	[
		...
		"(DPSO_taskID == '13') && ((position _this) call Gemini_fnc_isInsideBuilding) && (!spotted) && (_this call fnc_hasMicro) && (microInstallation < 24)", // Condition for the action to be shown
		"(DPSO_taskID == '13') && ((position _this) call Gemini_fnc_isInsideBuilding) && (!spotted) && (_this call fnc_hasMicro) && (microInstallation < 24)", // Condition for the action to progress
	...
	*/
	[
		...
		"(DPSO_taskID == '13') && ((position _this) call Gemini_fnc_isInsideBuilding) && (!spotted) && (_this call fnc_hasMicro) && (microInstallation < 24)", "(DPSO_taskID == '13') && ((position _this) call Gemini_fnc_isInsideBuilding) && (!spotted) && (_this call fnc_hasMicro) && (microInstallation < 24)",
...
// =========================================================================================================
	...
	if (random 100 > 50)
		then {[DPSO_enemy_side1, ["infantry"], selectRandom [0,2,2,3,3], _building, [10, 50], "talk", _building, DPSO_enemy_AIskill, 100, "task"] call Gemini_fnc_spawnSquad}
		else {[_building, 150, selectRandom [0,1,1,2,2], DPSO_enemy_side1, DPSO_enemy_units, DPSO_enemy_AIskill, "task"] call Gemini_fnc_spawnUnitsStandingInside};
	[DPSO_enemy_side1, ["infantry"], selectRandom [2,3], _building, [50, 100], "patrol", _building, DPSO_enemy_AIskill, 50, "task"] call Gemini_fnc_spawnSquad;
	[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,8], _building, [250, 500], "patrol", _building, DPSO_enemy_AIskill, 50, "task"] call Gemini_fnc_spawnSquad;
...
// =========================================================================================================
	...
	[
		...
		["DPSO_assignedTask && this", "friendsNearby = true", "friendsNearby = false"],
		[0, 0, 0, false],
		"task"
	...
	[
		...
		[0, 0, 0, false],
		["any", "present", false],
		["DPSO_assignedTask && (microInstallation >= 24) && !spotted && !friendsNearby", "[] remoteExec ['Gemini_fnc_taskSucceeded']; ['intel', 100] call Gemini_fnc_bonus", ""],
		[0, 0, 0, false],
		"task"
	] call Gemini_fnc_createTrigger;
...
// =========================================================================================================
	...
	[
		_building,
		[150, 150, 0, false],
		[DPSO_friendly_side3, DPSO_triggerDetectedByEnemy, false],
		["DPSO_assignedTask && this", "spotted = true; publicVariable 'spotted'; [] remoteExec ['Gemini_fnc_taskFailed']", ""],
		[0, 0, 0, false],
		"task"
	] call Gemini_fnc_createTrigger;