		...
		if ((_cave1_positions in _selectedCaves) && (random 10 > 2)) then
			{
				...
				while {_posIndex < _maxPos} do
					{
						if (random 10 > 7) then
							{
								_unit = [DPSO_enemy_side1, grpNull, DPSO_enemy_units, (_cave1_positions select _posIndex), DPSO_enemy_AIskill, false, "task"] call Gemini_fnc_createUnit;
				...
				[DPSO_enemy_side1, ["infantry"], selectRandom [2,3,5], _cave1_positions select 0, [0,250], "patrol", _cave1_positions select 0, DPSO_enemy_AIskill, 75, "task"] call Gemini_fnc_spawnSquad;
				[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,7,10,12], _cave1_positions select 0, [250,1000], "patrol", _cave1_positions select 0, DPSO_enemy_AIskill, 50, "task"] call Gemini_fnc_spawnSquad;

				// CREATING VEHICLE
				[DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _cave1_positions select 0, 100, [DPSO_enemy_units, selectRandom [0,1,2,3]], 10] call Gemini_fnc_spawnVehicle;
				...
				[_position, DPSO_enemy_statics + DPSO_enemy_MGstatics + DPSO_enemy_MGstatics, DPSO_enemy_side1, _azimuth, 0, selectRandom [1,2,3], 30, "task"] call Gemini_fnc_spawnStaticDefense;
		...
		if ((_cave2_positions in _selectedCaves) && (random 10 > 2)) then
			{
				...
				while {_posIndex < _maxPos} do
					{
						if (random 10 > 7) then
							{
								_unit = [DPSO_enemy_side1, grpNull, DPSO_enemy_units, (_cave2_positions select _posIndex), DPSO_enemy_AIskill, false, "task"] call Gemini_fnc_createUnit;
				...
				// CREATING SQUAD(S) AROUND CAVE
				[DPSO_enemy_side1, ["infantry"], selectRandom [2,3,5], _cave2_positions select 0, [0,250], "patrol", _cave2_positions select 0, DPSO_enemy_AIskill, 75, "task"] call Gemini_fnc_spawnSquad;
				[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,7,10,12], _cave2_positions select 0, [250,1000], "patrol", _cave2_positions select 0, DPSO_enemy_AIskill, 50, "task"] call Gemini_fnc_spawnSquad;

				// CREATING VEHICLE
				[DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _cave2_positions select 0, 100, [DPSO_enemy_units, selectRandom [0,1,2,3]], 10] call Gemini_fnc_spawnVehicle;
		...
		if ((_cave3_positions in _selectedCaves) && (random 10 > 2)) then
			{
					...
					while {_posIndex < _maxPos} do
						{
							if (random 10 > 7) then
								{
									_unit = [DPSO_enemy_side1, grpNull, DPSO_enemy_units, (_cave3_positions select _posIndex), DPSO_enemy_AIskill, false, "task"] call Gemini_fnc_createUnit;
				...
				// CREATING SQUAD(S) AROUND CAVE
				[DPSO_enemy_side1, ["infantry"], selectRandom [2,3,5], _cave3_positions select 0, [0,250], "patrol", _cave3_positions select 0, DPSO_enemy_AIskill, 75, "task"] call Gemini_fnc_spawnSquad;
				[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,7,10,12], _cave3_positions select 0, [250,1000], "patrol", _cave3_positions select 0, DPSO_enemy_AIskill, 50, "task"] call Gemini_fnc_spawnSquad;

				// CREATING VEHICLE
				[DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _cave3_positions select 0, 100, [DPSO_enemy_units, selectRandom [0,1,2,3]], 10] call Gemini_fnc_spawnVehicle;

				// CREATING STATIC DEFENSE
					private ["_randomPosition", "_position", "_azimuth"];
					[selectRandom _cave3_positions, DPSO_enemy_statics + DPSO_enemy_MGstatics + DPSO_enemy_MGstatics, DPSO_enemy_side1, random 360, 75, selectRandom [1,2,3], 30, "task"] call Gemini_fnc_spawnStaticDefense;
		...
		if ((_cave4_positions in _selectedCaves) && (random 10 > 2)) then
			{
				...
				while {_posIndex < _maxPos} do
					{
						if (random 10 > 7) then
							{
								_unit = [DPSO_enemy_side1, grpNull, DPSO_enemy_units, (_cave4_positions select _posIndex), DPSO_enemy_AIskill, false, "task"] call Gemini_fnc_createUnit;
				...
				// CREATING SQUAD(S) AROUND CAVE
				[DPSO_enemy_side1, ["infantry"], selectRandom [2,3,5], _cave4_positions select 0, [0,250], "patrol", _cave4_positions select 0, DPSO_enemy_AIskill, 75, "task"] call Gemini_fnc_spawnSquad;
				[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,7,10,12], _cave4_positions select 0, [250,1000], "patrol", _cave4_positions select 0, DPSO_enemy_AIskill, 50, "task"] call Gemini_fnc_spawnSquad;

				// CREATING VEHICLE
				[DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _cave4_positions select 0, 100, [DPSO_enemy_units, selectRandom [0,1,2,3]], 10] call Gemini_fnc_spawnVehicle;
		...
		if ((_cave5_positions in _selectedCaves) && (random 10 > 2)) then
			{
				...
				while {_posIndex < _maxPos} do
					{
						if (random 10 > 8) then
							{
								_unit = [DPSO_enemy_side1, grpNull, DPSO_enemy_units, (_cave5_positions select _posIndex), DPSO_enemy_AIskill, false, "task"] call Gemini_fnc_createUnit;
				...
				// CREATING SQUAD(S) AROUND CAVE
				[DPSO_enemy_side1, ["infantry"], selectRandom [2,3,5], _cave5_positions select 0, [0,250], "patrol", _cave5_positions select 0, DPSO_enemy_AIskill, 75, "task"] call Gemini_fnc_spawnSquad;
				[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,7,10,12], _cave5_positions select 0, [250,1000], "patrol", _cave5_positions select 0, DPSO_enemy_AIskill, 50, "task"] call Gemini_fnc_spawnSquad;

				// CREATING VEHICLE
				[DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _cave5_positions select 0, 100, [DPSO_enemy_units, selectRandom [0,1,2,3]], 10] call Gemini_fnc_spawnVehicle;;

				// CREATING STATIC DEFENSE
				[selectRandom _cave5_positions, DPSO_enemy_statics + DPSO_enemy_MGstatics + DPSO_enemy_MGstatics, DPSO_enemy_side1, random 360, 75, selectRandom [1,2,3], 30, "task"] call Gemini_fnc_spawnStaticDefense;
		...
		if ((_cave6_positions in _selectedCaves) && (random 10 > 2)) then
			{
				...
				while {_posIndex < _maxPos} do
					{
						if (random 10 > 7) then
							{
								_unit = [DPSO_enemy_side1, grpNull, DPSO_enemy_units, (_cave6_positions select _posIndex), DPSO_enemy_AIskill, false, "task"] call Gemini_fnc_createUnit;
				...
				// CREATING SQUAD(S) AROUND CAVE
				[DPSO_enemy_side1, ["infantry"], selectRandom [2,3,5], _cave6_positions select 0, [0,250], "patrol", _cave6_positions select 0, DPSO_enemy_AIskill, 75, "task"] call Gemini_fnc_spawnSquad;
				[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,7,10,12], _cave6_positions select 0, [250,1000], "patrol", _cave6_positions select 0, DPSO_enemy_AIskill, 50, "task"] call Gemini_fnc_spawnSquad;

				// CREATING VEHICLE
				[DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _cave6_positions select 0, 100, [DPSO_enemy_units, selectRandom [0,1,2,3]], 10] call Gemini_fnc_spawnVehicle;

				// CREATING STATIC DEFENSE
				[selectRandom _cave6_positions, DPSO_enemy_statics + DPSO_enemy_MGstatics + DPSO_enemy_MGstatics, DPSO_enemy_side1, random 360, 75, selectRandom [1,2,3], 30, "task"] call Gemini_fnc_spawnStaticDefense;
		...
		if ((_cave7_positions in _selectedCaves) && (random 10 > 2)) then
			{
				...
				while {_posIndex < _maxPos} do
					{
						if (random 10 > 7) then
							{
								_unit = [DPSO_enemy_side1, grpNull, DPSO_enemy_units, (_cave7_positions select _posIndex), DPSO_enemy_AIskill, false, "task"] call Gemini_fnc_createUnit;
				...
				// CREATING SQUAD(S) AROUND CAVE
				[DPSO_enemy_side1, ["infantry"], selectRandom [2,3,5], _cave7_positions select 0, [0,250], "patrol", _cave7_positions select 0, DPSO_enemy_AIskill, 75, "task"] call Gemini_fnc_spawnSquad;
				[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,7,10,12], _cave7_positions select 0, [250,1000], "patrol", _cave7_positions select 0, DPSO_enemy_AIskill, 50, "task"] call Gemini_fnc_spawnSquad;

				// CREATING VEHICLE
				[DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _cave7_positions select 0, 100, [DPSO_enemy_units, selectRandom [0,1,2,3]], 10] call Gemini_fnc_spawnVehicle;

				// CREATING STATIC DEFENSE
				[selectRandom _cave7_positions, DPSO_enemy_statics + DPSO_enemy_MGstatics + DPSO_enemy_MGstatics, DPSO_enemy_side1, random 360, 75, selectRandom [1,2,3], 30, "task"] call Gemini_fnc_spawnStaticDefense;
		...
		if ((_cave8_positions in _selectedCaves) && (random 10 > 2)) then
			{
				...
				while {_posIndex < _maxPos} do
					{
						if (random 10 > 7) then
							{
								_unit = [DPSO_enemy_side1, grpNull, DPSO_enemy_units, (_cave8_positions select _posIndex), DPSO_enemy_AIskill, false, "task"] call Gemini_fnc_createUnit;
				...
				// CREATING SQUAD(S) AROUND CAVE
				[DPSO_enemy_side1, ["infantry"], selectRandom [2,3,5], _cave8_positions select 0, [0,250], "patrol", _cave8_positions select 0, DPSO_enemy_AIskill, 75, "task"] call Gemini_fnc_spawnSquad;
				[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,7,10,12], _cave8_positions select 0, [250,1000], "patrol", _cave8_positions select 0, DPSO_enemy_AIskill, 50, "task"] call Gemini_fnc_spawnSquad;

				// CREATING VEHICLE
				[DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _cave8_positions select 0, 100, [DPSO_enemy_units, selectRandom [0,1,2,3]], 10] call Gemini_fnc_spawnVehicle;

				// CREATING STATIC DEFENSE
				[selectRandom _cave8_positions, DPSO_enemy_statics + DPSO_enemy_MGstatics + DPSO_enemy_MGstatics, DPSO_enemy_side1, random 360, 75, selectRandom [1,2,3], 30, "task"] call Gemini_fnc_spawnStaticDefense;
		...
		if ((_cave9_positions in _selectedCaves) && (random 10 > 2)) then
			{
				...
				while {_posIndex < _maxPos} do
					{
						if (random 10 > 7) then
							{
								_unit = [DPSO_enemy_side1, grpNull, DPSO_enemy_units, (_cave9_positions select _posIndex), DPSO_enemy_AIskill, false, "task"] call Gemini_fnc_createUnit;
				...
				// CREATING SQUAD(S) AROUND CAVE
				[DPSO_enemy_side1, ["infantry"], selectRandom [2,3,5], _cave9_positions select 0, [0,250], "patrol", _cave9_positions select 0, DPSO_enemy_AIskill, 75, "task"] call Gemini_fnc_spawnSquad;
				[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,7,10,12], _cave9_positions select 0, [250,1000], "patrol", _cave9_positions select 0, DPSO_enemy_AIskill, 50, "task"] call Gemini_fnc_spawnSquad;

				// CREATING VEHICLE
				[DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _cave9_positions select 0, 100, [DPSO_enemy_units, selectRandom [0,1,2,3]], 10] call Gemini_fnc_spawnVehicle;

				// CREATING STATIC DEFENSE
				[selectRandom _cave9_positions, DPSO_enemy_statics + DPSO_enemy_MGstatics + DPSO_enemy_MGstatics, DPSO_enemy_side1, random 360, 75, selectRandom [1,2,3], 30, "task"] call Gemini_fnc_spawnStaticDefense;
			...
			for "_i" from 1 to (ceil (random 2)) do
				{
					...
					_civilian = [DPSO_civilian_side1, grpNull, DPSO_civilian_units, _civilianPos, DPSO_civilian_AIskill, false, "task"] call Gemini_fnc_createUnit;
					_civilian forceAddUniform (DPSO_civilian_uniforms call BIS_fnc_selectRandom);
					_civilian addHeadGear (DPSO_civilian_headgears call BIS_fnc_selectRandom);
...
// =========================================================================================================
	...
	private _clearTrigger1 =
		[
			...
			[100, 100, 0, false],
			[DPSO_enemy_side2, "NOT PRESENT", true],
	...
	private _presenceTrigger1 =
		[
			_selectedCaves select 0 select 0,
			[10, 10, 0, false],
			[DPSO_friendly_side3, "PRESENT", true],
	...
	_selectedCaves spawn
		{
			...
			while {DPSO_assignedTask} do
				{
					sleep 3;
					if (DPSO_assignedTask && cave1_cleared && cave1_visited) then {"taskMarker_A" setMarkerSize [0,0]; "taskMarker_B" setMarkerType "mil_objective"; "taskMarker_B" setMarkerPos ((_selectedCaves select 0) select 0); "taskMarker_B" setMarkerText localize "STR_hideoutCleared"; "taskMarker_B" setMarkerColor "colorGreen"};
					if (DPSO_assignedTask && cave2_cleared && cave2_visited) then {"taskMarker_C" setMarkerSize [0,0]; "taskMarker_D" setMarkerType "mil_objective"; "taskMarker_D" setMarkerPos ((_selectedCaves select 1) select 0); "taskMarker_D" setMarkerText localize "STR_hideoutCleared"; "taskMarker_D" setMarkerColor "colorGreen"};
					if (DPSO_assignedTask && cave3_cleared && cave3_visited) then {"taskMarker_E" setMarkerSize [0,0]; "taskMarker_F" setMarkerType "mil_objective"; "taskMarker_F" setMarkerPos ((_selectedCaves select 2) select 0); "taskMarker_F" setMarkerText localize "STR_hideoutCleared"; "taskMarker_F" setMarkerColor "colorGreen"};

					// TASK SUCCEEDED IF ALL 3 CACHES HAS BEEN CLEARED AND VISITED
					if (DPSO_assignedTask && cave1_cleared && cave1_visited && cave2_cleared && cave2_visited && cave3_cleared && cave3_visited) then {[] remoteExecCall ["Gemini_fnc_taskSucceeded"]; ["supply", "explosives"] call Gemini_fnc_bonus};
				};
		};
...
// =========================================================================================================
	...
	[DPSO_taskID, "attack", "taskMarker_A"] remoteExecCall ["Gemini_fnc_taskAssigned"];
