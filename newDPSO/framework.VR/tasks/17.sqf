...
// =========================================================================================================
	...
	private _emptyPos = [];
	_emptyPos = (position DPSO_deliveryZone) isFlatEmpty [1, 1, 0.75, 10, 0, false];
...
// =========================================================================================================
	...
	private _locationPos = [["city", "city"], DPSO_mapCenter, 0, DPSO_mapRadius] call Gemini_fnc_findPos;
...
// =========================================================================================================
	...
	private _supply_medical = ["Land_PaperBox_01_small_closed_white_med_F", [(getPos DPSO_deliveryZone select 0) + 0, (getPos DPSO_deliveryZone select 1) + 0, 0], random 360, false, "task"] call Gemini_fnc_createVehicle;
	private _supply_food1 = ["Land_Sacks_heap_F", [(getPos DPSO_deliveryZone select 0) - 1.5, (getPos DPSO_deliveryZone select 1) + 1.5, 0], random 360, false, "task"] call Gemini_fnc_createVehicle;
	private _supply_food2 = ["Land_Sacks_goods_F", [(getPos DPSO_deliveryZone select 0) - 1.5, (getPos DPSO_deliveryZone select 1) - 1.5, 0], random 360, false, "task"] call Gemini_fnc_createVehicle;
	private _supply_water1 = ["Land_BarrelWater_F", [(getPos DPSO_deliveryZone select 0) + 1.5, (getPos DPSO_deliveryZone select 1) + 1.5, 0], random 360, false, "task"] call Gemini_fnc_createVehicle;
	private _supply_water2 = ["Land_BarrelWater_F", [(getPos DPSO_deliveryZone select 0) + 1.5, (getPos DPSO_deliveryZone select 1) - 1.5, 0], random 360, false, "task"] call Gemini_fnc_createVehicle;
			...
			while {DPSO_assignedTask} do
				{
					...
					if (DPSO_assignedTask && (alive _supply_food1) && ([_locationTrigger, _supply_food1] call BIS_fnc_inTrigger) && (isNull attachedTo _supply_food1)) then {_deliveredSupplies pushBack _supply_food1};
					if (DPSO_assignedTask && (alive _supply_food2) && ([_locationTrigger, _supply_food2] call BIS_fnc_inTrigger) && (isNull attachedTo _supply_food2)) then {_deliveredSupplies pushBack _supply_food2};
					if (DPSO_assignedTask && (alive _supply_water1) && ([_locationTrigger, _supply_water1] call BIS_fnc_inTrigger) && (isNull attachedTo _supply_water1)) then {_deliveredSupplies pushBack _supply_water1};
					if (DPSO_assignedTask && (alive _supply_water2) && ([_locationTrigger, _supply_water2] call BIS_fnc_inTrigger) && (isNull attachedTo _supply_water2)) then {_deliveredSupplies pushBack _supply_water2};
					if (DPSO_assignedTask && (alive _supply_medical) && ([_locationTrigger, _supply_medical] call BIS_fnc_inTrigger) && (isNull attachedTo _supply_medical)) then {_deliveredSupplies pushBack _supply_medical};
					if (DPSO_assignedTask && (count _civilians > 0) && (count _deliveredSupplies > 0)) then
						{
							{
								if ((DPSO_assignedTask) && (alive _x) && (random 100 < 25)) then
									{
	...
	[_locationPos, _locationRadius] spawn
		{
			...
			private _ambushCenter = [(((position DPSO_deliveryZone) select 0) + (_locationPos select 0)) / 2, (((position DPSO_deliveryZone) select 1) + (_locationPos select 1)) / 2, 0];
			private _ambushRadius = (_locationPos distance2D DPSO_deliveryZone) / 2.5;
			...
			if (!(_ambushPos isEqualTo [0,0,0])) then {[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,7,8,10,12,15,20], _ambushPos, [0,0], "hold", _ambushCenter, DPSO_enemy_AIskill, 80, "task"] call Gemini_fnc_spawnSquad};

			// SPAWNING A PATROL AROUND DESTINATION
			[DPSO_enemy_side1, ["infantry"], selectRandom [3,5,8,12], _locationPos, [_locationRadius * 2, _locationRadius * 5], "patrol", _locationPos, DPSO_enemy_AIskill, 15, "task"] call Gemini_fnc_spawnSquad;
		};
...
// =========================================================================================================
	...
	if (DPSO_debug) then {DPSO_task17_supplies = [_supply_food1, _supply_food2, _supply_water1, _supply_water2, _supply_medical]; publicVariable "DPSO_task17_supplies"};
	...
	while {DPSO_assignedTask} do
		{
			...
			if (DPSO_assignedTask && ((!alive _supply_food1) || (!alive _supply_food2) || (!alive _supply_water1) || (!alive _supply_water2) || (!alive _supply_medical))) then {[] remoteExec ["Gemini_fnc_taskFailed"]};
