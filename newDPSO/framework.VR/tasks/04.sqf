...
// =========================================================================================================s
	...
	private _mainPos = ["land", DPSO_mapCenter, 0, DPSO_mapRadius] call Gemini_fnc_findPos;
...
// =========================================================================================================
	...
	private _trigger1 =
		[
			_patrolPos1,
			[15, 15, 0, false],
			[DPSO_friendly_side3, "present", false],
			["", "", ""],
			[0, 0, 0, false],
			"task"
		] call Gemini_fnc_createTrigger;
	_trigger1 setTriggerStatements ["{this && DPSO_assignedTask && (((getPos _x) select 2) < 3)} forEach thisList", "checkpoint1_cleared = true; 'taskMarker_A' setMarkerColor 'colorGreen'", ""];
	...
	_trigger2 setTriggerStatements ["{this && DPSO_assignedTask && (((getPos _x) select 2) < 3)} forEach thisList", "checkpoint2_cleared = true; 'taskMarker_B' setMarkerColor 'colorGreen'", ""];
	...
	_trigger3 setTriggerStatements ["{this && DPSO_assignedTask && (((getPos _x) select 2) < 3)} forEach thisList", "checkpoint3_cleared = true; 'taskMarker_C' setMarkerColor 'colorGreen'", ""];
	...
	_trigger4 setTriggerStatements ["{this && DPSO_assignedTask && (((getPos _x) select 2) < 3)} forEach thisList", "checkpoint4_cleared = true; 'taskMarker_D' setMarkerColor 'colorGreen'", ""];
	...
	_trigger5 setTriggerStatements ["{this && DPSO_assignedTask && (((getPos _x) select 2) < 3)} forEach thisList", "checkpoint5_cleared = true; 'taskMarker_E' setMarkerColor 'colorGreen'", ""];
...
// =========================================================================================================
	...
	{
		[DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _x, _taskRadius / 6, [DPSO_enemy_units, selectRandom [2,3,4,5]], 5, "task"] call Gemini_fnc_spawnVehicle;
		[DPSO_enemy_side1, ["infantry"], -1, _x, _taskRadius / 6, "patrol", _x, DPSO_enemy_AIskill, 10, "task"] call Gemini_fnc_spawnSquad;
		if (random 100 > 90) then {[DPSO_enemy_side1, ["infantry"], selectRandom [2,3], _x, _taskRadius / 6, "talk", _x, DPSO_enemy_AIskill, 10, "task"] call Gemini_fnc_spawnSquad};
		if (random 100 > 90) then {[_x, _taskRadius / 6, -1, DPSO_enemy_side1, DPSO_enemy_units, DPSO_enemy_AIskill, "task"] call Gemini_fnc_spawnUnitsStandingInside};
	...
	[] spawn
		{
			while {DPSO_assignedTask} do
				{
					sleep 3;
					if (DPSO_assignedTask && checkpoint1_cleared && checkpoint2_cleared && checkpoint3_cleared && checkpoint4_cleared && checkpoint5_cleared) then {[] remoteExec ["Gemini_fnc_taskSucceeded"]};
				};
		};
...
// =========================================================================================================
	...
	[DPSO_taskID, "walk", nil] remoteExec ["Gemini_fnc_taskAssigned"];
