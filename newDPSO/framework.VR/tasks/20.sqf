...
//  /!\ THIS DPSO_task IS NOT WORKING PROPERLY YET - ARMA AI IS SO STUPID WHEN DRIVING VEHICLES, MORE ADJUSTMENTS ARE REQUIRED !!!
// =========================================================================================================
...
// =========================================================================================================
	...
	private _convoyDestination = [["industry", "military"], _roadPos, 2000, DPSO_mapRadius] call Gemini_fnc_findPos;
	...
	[_convoyStart, _convoyDestination, _convoyDirection] spawn
		{
			...
			waitUntil {(!DPSO_assignedTask) || ({_convoyStart distance _x < 2000} count (playableUnits + switchableUnits) > 0) || ({_convoyDestination distance _x < 2000} count (playableUnits + switchableUnits) > 0)};
			if (!DPSO_assignedTask) exitWith {[] remoteExec ["Gemini_fnc_taskReset"]};

			// SPAWNING VEHICLE & SQUAD 1
			private _vehicle1 = [DPSO_enemy_combatCars + DPSO_enemy_transportCars, _convoyStart, _convoyDirection, true, "task"] call Gemini_fnc_createVehicle;
			...
			private _squad1 = [DPSO_enemy_side1, ["infantry"], _availablePositions1, _convoyStart, 50, "", _convoyDestination, DPSO_enemy_AIskill, 100, "task"] call Gemini_fnc_spawnSquad;
			{
			...
			waitUntil {!DPSO_assignedTask || (_vehicle1 distance _convoyStart > 20)};
			private _vehicle2 = [DPSO_enemy_combatCars + DPSO_enemy_combatCars + DPSO_enemy_combatCars + DPSO_enemy_armored, _convoyStart, getDir _vehicle1, true, "task"] call Gemini_fnc_createVehicle;
			...
			private _squad2 = [DPSO_enemy_side1, ["infantry"], _availablePositions2, _convoyStart, 50, "", _convoyDestination, DPSO_enemy_AIskill, 100, "task"] call Gemini_fnc_spawnSquad;
			{
			...
			waitUntil {!DPSO_assignedTask || (_vehicle2 distance _convoyStart > 20)};
			private _vehicle3 = [DPSO_enemy_transportTrucks, _convoyStart, getDir _vehicle2, true, "task"] call Gemini_fnc_createVehicle;
			{_vehicle3 enablePersonTurret [_x, false]} forEach (allTurrets _vehicle3);
			private _availablePositions3 = 3;
			private _squad3 = [DPSO_enemy_side1, ["infantry"], _availablePositions3, _convoyStart, 50, "", _convoyDestination, DPSO_enemy_AIskill, 100, "task"] call Gemini_fnc_spawnSquad;
			{
			...
			waitUntil {!DPSO_assignedTask || (_vehicle3 distance _convoyStart > 20)};
			private _vehicle4 = [DPSO_enemy_transportTrucks, _convoyStart, getDir _vehicle3, true, "task"] call Gemini_fnc_createVehicle;
			{_vehicle4 enablePersonTurret [_x, false]} forEach (allTurrets _vehicle4);
			private _availablePositions4 = 3;
			private _squad4 = [DPSO_enemy_side1, ["infantry"], _availablePositions4, _convoyStart, 50, "", _convoyDestination, DPSO_enemy_AIskill, 100, "task"] call Gemini_fnc_spawnSquad;
			{
			...
			waitUntil {!DPSO_assignedTask || (_vehicle4 distance _convoyStart > 20)};
			private _vehicle5 = [DPSO_enemy_combatCars + DPSO_enemy_combatCars + DPSO_enemy_transportCars, _convoyStart, getDir _vehicle4, true, "task"] call Gemini_fnc_createVehicle;
			...
			private _squad5 = [DPSO_enemy_side1, ["infantry"], _availablePositions5, _convoyStart, 50, "", _convoyDestination, DPSO_enemy_AIskill, 100, "task"] call Gemini_fnc_spawnSquad;
			{
			...
			{
				private _launcher = selectRandom (DPSO_enemy_AAlaunchers + DPSO_enemy_ATlaunchers);
			...
			[_vehicle3, _vehicle4] spawn
				{
					...
					while {DPSO_assignedTask && (alive _vehicle3 || alive _vehicle4) && (count (((getWeaponCargo _vehicle3) select 0) + ((getMagazineCargo _vehicle3) select 0) + ((getWeaponCargo _vehicle4) select 0) + ((getMagazineCargo _vehicle4) select 0)) > 0)} do {sleep 5};
					if (DPSO_assignedTask && ((!alive _vehicle3 && !alive _vehicle4) || (count (((getWeaponCargo _vehicle3) select 0) + ((getMagazineCargo _vehicle3) select 0) + ((getWeaponCargo _vehicle4) select 0) + ((getMagazineCargo _vehicle4) select 0)) == 0))) then
						{
			...
			[_vehicle3, _vehicle4, _convoyDestination] spawn
				{
					...
					while {DPSO_assignedTask} do
						{
							sleep 10;
							if (DPSO_assignedTask && ((_vehicle3 distance _convoyDestination < 25) || (_vehicle4 distance _convoyDestination < 25))) then {[] remoteExec ["Gemini_fnc_taskFailed"]};
						};
				};

			// PROCESSING CONVOY
			convoy_20 = [_vehicle1, _vehicle2, _vehicle3, _vehicle4, _vehicle5];
			convoy_20 spawn {while {DPSO_assignedTask} do {sleep 3; if (count convoy_20 > 0) then {{if ((!canMove _x) && (_x in convoy_20)) then {convoy_20 = convoy_20 - [_x]}} forEach convoy_20}}};
			...
			while {DPSO_assignedTask && canMove _vehicle3 && canMove _vehicle4 && canMove driver_20_3 && canMove driver_20_4} do
				{
					sleep 1;
...
// =========================================================================================================
	...
	private _defenseVehicle1 = [DPSO_enemy_armored + DPSO_enemy_motorizedVehicles + DPSO_enemy_motorizedVehicles + DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _convoyDestination, _destinationSize, [DPSO_enemy_units, ceil (random 3)], 80, "task"] call Gemini_fnc_spawnVehicle;
	private _defenseVehicle2 = [DPSO_enemy_armored + DPSO_enemy_motorizedVehicles + DPSO_enemy_motorizedVehicles + DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _convoyDestination, _destinationSize, [DPSO_enemy_units, ceil (random 3)], 40, "task"] call Gemini_fnc_spawnVehicle;
	private _defenseVehicle3 = [DPSO_enemy_armored + DPSO_enemy_motorizedVehicles + DPSO_enemy_motorizedVehicles + DPSO_enemy_motorizedVehicles, DPSO_enemy_side1, _convoyDestination, _destinationSize, [DPSO_enemy_units, ceil (random 3)], 20, "task"] call Gemini_fnc_spawnVehicle;

	// SPAWNING STATIC ENEMIES INSIDE BUILDINGS	(IF THERE ARE ANY)
	[_convoyDestination, _destinationSize, -1, DPSO_enemy_side1, DPSO_enemy_units, DPSO_enemy_AIskill, "task"] call Gemini_fnc_spawnUnitsStandingInside;

	// SPAWNING STATIC ENEMIES OUTSIDE
	[_convoyDestination, _destinationSize, -1, DPSO_enemy_side1, DPSO_enemy_units, DPSO_enemy_AIskill, "task"] call Gemini_fnc_spawnUnitsStandingOutside;
	for "_i" from 1 to (selectRandom [1,2,3]) do {[DPSO_enemy_side1, ["infantry"], selectRandom [2,3], _convoyDestination, [0, _destinationSize], "talk", _convoyDestination, DPSO_enemy_AIskill, 75, "task"] call Gemini_fnc_spawnSquad};

	//  SPAWNING SQUAD(S) PATROLLING THE AREA
	for "_i" from 1 to (selectRandom [1,2,3]) do {[DPSO_enemy_side1, ["infantry"], -1, _convoyDestination, [0, _destinationSize * 2], "patrol", _convoyDestination, DPSO_enemy_AIskill, 75, "task"] call Gemini_fnc_spawnSquad};
...
// =========================================================================================================
	...
	["taskMarker_C", (format ["STR_taskObjective_" + DPSO_taskID])] remoteExec ["Gemini_fnc_setMarkerText"];
...
// =========================================================================================================
	...
	[DPSO_taskID, "attack", "taskMarker_C"] remoteExec ["Gemini_fnc_taskAssigned"];
