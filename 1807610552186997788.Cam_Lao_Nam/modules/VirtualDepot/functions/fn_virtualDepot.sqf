...
#define ICONACTIONDEPOT		"framework\data\holdactions\holdAction_depot_ca.paa"
#define ICONACTIONREPAIR	"framework\data\holdactions\holdAction_repair_ca.paa"
#define ICONREPAIRMISSILE	"framework\data\holdactions\holdAction_missile_ca.paa"
#define ICONACTIONMAGAZINE	"framework\data\holdactions\holdAction_magazine_ca.paa"
...
// UI
	...
	case "ctrlListPylonUpdate" : {
		...
		private _addMags = ["CfgVirtualDepot","addMagsInDepot"] call MPSF_fnc_getCfgDataArray;
		private _allPylonMagazines = "toLower (configname _x) find 'pylon' >= 0 || (configName _x) in _addMags" configClasses (configfile >> "CfgMagazines");
		{
	...
	case "onUnloadDepotUI" : {
		...
		private _center = ["getTargetCenter"] call MPSF_fnc_virtualDepot;
		["allowDamage",[_center,false]] call MPSF_fnc_virtualDepot;
		["closeVirtualDepot",[_depotID,player,false],side player] call MPSF_fnc_triggerEventHandler;
	...
	case "openDepotUI" : {
		...
		private _activeUser = _depotLogic getVariable ["ActiveUser",objNull];
		if !(alive _activeUser && isPlayer _activeUser) then {_activeUser = objNull};
		if !(isNull _activeUser) exitWith { /*["Depot is already in USE"] call BIS_fnc_error;*/ };
...
// Dynamic Aircraft Loadout Editor (DALE) Functions
	...
	case "toggleBay" : { // Broken
		...
		_vehicle animateBay [_pylonID,_pylonPhase,_pylonPhase < 0];
	};
	case "togglePylon" : { // Broken
		...
		_vehicle animatePylon [_pylonID,_pylonPhase,_pylonPhase < 0];
	};
...
// Functions
	...
	case "deployCenter" : {
		...
		if (["isMHQ",[_center]] call MPSF_fnc_virtualDepot) then {
			["onMHQinit",[_center,(side group player) call BIS_fnc_sideID],0] call MPSF_fnc_triggerEventHandler;
		};
		_center setFuel 1;
	};
	case "setDeployed" : {
	...
	case "nearbyDepot" : {
		...
		private _nearby = player distance2D _depotLogic < 15;
		if !(_nearby) then {["deactivateDepot"] call MPSF_fnc_virtualDepot};
		_nearby
	};
	...
	case "preLoadVehicles" : {
...

		{
			...
			private _configName = configname _cfgObject;
			private _prefixMod = toLower (_configName select [0,3]) in ["vn_"];
			private _parents = ([_cfgObject,true] call BIS_fnc_returnParents) apply {toLower _x};
			if (count (_parents arrayIntersect _classTypes) > 0 && _prefixMod) then {
...
// Actions
	case "createTriggerAction" : {
		diag_log format ["MPSF_fnc_VirtualDepot %1 with %2",toUpper _mode,_params];
	...
	case "activateDepot" : {
		...
		if (/* ["getCfgDepotPylonInteraction"] call MPSF_fnc_virtualDepot */false) then {
			...
			["MPSF_VirtualDepot_openUI",player,"Access Virtual Depot",[
					...
					+ format [" && ['nearbyDepot',[%1]] call MPSF_fnc_virtualDepot",str (_depotLogic call BIS_fnc_netId)]
			...
			["MPSF_DALE_loadMag_action",player,"Load Ordinance",[
					[ICONACTIONMAGAZINE,{ ["setPylonMagAmmoPercent",[nil,nil,0]] call MPSF_fnc_virtualDepot; }]
					,[ICONACTIONMAGAZINE,{ ["setPylonMagAmmoPercent",[nil,nil,(_this select 4)/24]] call MPSF_fnc_virtualDepot; }]
...
// Configuration
	...
	case "initDiary" : {
		["MPSF_Framework_Note","Features","Vehicle Depot"
			,format["<font size='16' face='PuristaLight'>Virtual Vehicle Depot</font><br/><img image='#(argb,8,8,3)color(1,1,1,0.1)' height='1' width='640' /><br/>%1<br/><br/>%2<br/><br/>%3<br/><br/>%4"
...
// Init
	case "initLogic" : {
		...
		["onVehicleDepotCreate",[_depotLogic,_target],0] spawn {sleep 1; _this call MPSF_fnc_triggerEventHandler; };
	};
	case "preInit";
	case "postInit" : {
		// Init Supply Points
		...
		// Diary
		["initDiary"] call MPSF_fnc_virtualDepot;
	};
};