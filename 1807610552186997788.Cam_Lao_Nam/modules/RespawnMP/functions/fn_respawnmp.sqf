	...
	case "setPosition" : {
		...
		switch (true) do {
			case (_position isEqualType objNull) : {
				...
				private _offsetDist = if (vehicle _target isEqualTo _target) then { 1.5 } else { 9 };
				//--- vehicle is on the ground, move next to it
				private _offset = (getPos _position distance getPosVisual _position) + _offsetDist;
				...
				if (vehicle _target isEqualTo _target) then {
					...
					private _movePos = _spawnPos findEmptyPosition [15,35,typeOf vehicle _target];
					_vehicle allowDamage false;
					_vehicle setVehiclePosition [_movePos, [], 0, "NONE"];
					_vehicle setDir (_target getDir _position);
...
// Trigger
	...
	case "activateAction" : {
		...
		["MPSF_RespawnMP_Action",vehicle player,format["<t color='#ffC600'>Redeploy %1</t>",if (vehicle player isEqualTo player) then {"Soldier"} else {[vehicle player] call MPSF_fnc_getCfgText}],[
		...
		,true,false,100] spawn {sleep 0.1; _this call MPSF_fnc_addAction;};
	};
                ...
                private _centerPos = _position call BIS_fnc_position;
                {
                    ...
                    if (local _target) then {
                        switch (true) do {
							...
							case (!(vehicle _target isEqualTo _target) && !(driver vehicle _target isEqualTo _target)) : {
								// Do Nothing
							};
							case (driver vehicle _target isEqualTo _target) : {
                                ["setPosition",[_target,_position]] call MPSF_fnc_respawnMP;
							};
                            default {
                                ["setPosition",[_target,_position]] call MPSF_fnc_respawnMP;
