...
switch (_mode) do {
	...
	case "canDeploy" : {
        ...
        if !(["isEnabled"] call MPSF_fnc_rallypoint) exitWith {false};
		...
		vehicle _target isEqualTo _target && (isTouchingGround _target) && (speed _target < 1);
	};
	case "postInit" : {
        ...
        ["MPSF_RespawnMP_onDeployRallyPoint_EH","onDeployRallyPoint",{
			...
			if (isServer) then {
				...
				private _handlerID = [side _owner,_target,format["Rally #%1",mapGridPosition _target]] call BIS_fnc_addRespawnPosition;
