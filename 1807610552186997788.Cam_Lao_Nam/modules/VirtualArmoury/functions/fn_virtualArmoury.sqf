...
// Configuration
	...
	case "getCompatibleRoleWeapons" : {
		...
		if !(isNil ("MPSF_ArmouryWeaponTypes_type%" + _roleID)) exitWith {
			missionNamespace getVariable [format ["MPSF_ArmouryWeaponTypes_type%",_roleID],[[],[],[],[]]];
		};

		private _types = (["CfgVirtualArmoury",_roleID,"weaponTypes"] call MPSF_fnc_getCfgDataArray) apply {toLower _x};
		private _list = [[],[],[],[],[]];
		private _itemFilter = [
...

		{
			...
			private _prefixMod = toLower (_weapon select [0,3]) in ["vn_"];
			...
			if ((toLower _weaponTypeSpecific in _types || count _types isEqualTo []) && _prefixMod) then {
				{
					private _itemFilter_list = _x apply {toLower _x};
					if (toLower _weaponTypeSpecific in _itemFilter_list || _itemFilter_list isEqualTo []) then {
		...
		//--- Magazines - Put and Throw
		{
			private _weapon  = _x;
			{
				{
					private _mag = toLower _x;
					private _prefixMod = toLower (_mag select [0,3]) in ["vn_"];
					private _cfgMag = _cfgMagazines >> _mag;
					if ((getNumber (_cfgMag >> "scope") == 2 || getNumber (_cfgMag >> "scopeArsenal") == 2) && _prefixMod) then {
						_list select 4 pushBackUnique _mag;
					};
		...
		//--- Backpacks
		{
			...
			_weapon call bis_fnc_itemType params ["", "_weaponTypeSpecific"];
			{
				if (_weaponTypeSpecific isEqualTo _x) exitWith {
					if (getnumber (_weaponCfg >> "scope") == 2 && _prefixMod) then {
	...
	case "init": {
		...
		};
		{
			...
			if (count (_virtualArmouryBox getVariable [format["%1",_roleID],[]]) isEqualTo 0) then {
				private _weaponsList = ["getCompatibleRoleWeapons",[_roleID]] call MPSF_fnc_virtualArmoury;
				_virtualArmouryBox setVariable [format["%1",_roleID],_weaponsList];
			};
		} forEach (["getCfgRoles",[-1]] call MPSF_fnc_getCfgRoles);
	};
	case "postInit" : {
		/* Respawn Loadout Event Handler */
		missionNamespace setVariable ["BIS_fnc_arsenal_fullArsenal",false,true];
