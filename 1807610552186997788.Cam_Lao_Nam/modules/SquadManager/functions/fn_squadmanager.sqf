...
switch (_mode) do {
	case "onLoadSquadMenu" : {
		...
		["setCtrlText",["RscControlAmrelTabletMenuScreen_Fnc1","Tab 1: Squads"]] call MPSF_fnc_itemAmrelTablet;
		["setCtrlText",["RscControlAmrelTabletMenuScreen_Fnc2","Tab 2: Team"]] call MPSF_fnc_itemAmrelTablet;
		["setCtrlText",["RscControlAmrelTabletMenuScreen_Fnc3","Tab 3: Roles"]] call MPSF_fnc_itemAmrelTablet;
		["setCtrlText",["RscControlAmrelTabletMenuScreen_Fnc4","Tab 4: Empty"]] call MPSF_fnc_itemAmrelTablet;
		["setCtrlText",["RscControlAmrelTabletMenuScreen_Fnc5","Tab 5: Empty"]] call MPSF_fnc_itemAmrelTablet;
		["setCtrlText",["RscControlAmrelTabletMenuScreen_Fnc6","Tab 6: Empty"]] call MPSF_fnc_itemAmrelTablet;
	...
	case "RefreshSquadUI" : {
		...
		["FillGroupsListbox",[
			["GetAllGroupsOfSide",[side group player]] call MPSF_fnc_dynamicGroups,
		...
		["FillPlayersListbox",[
			if ([player] call MPSF_fnc_isAdmin) then {
				["GetFriendlyPlayers",[side group player]] call MPSF_fnc_dynamicGroups
			} else {
				units group player
			},
			false,
		...
		["FillRoleComboBox",[side group player]] call MPSF_fnc_squadManager;
		["FillRankComboBox",[side group player]] call MPSF_fnc_squadManager;
	...
	case "RefreshSquadScreen" : {
		...
		private _insigniaTexture = if (_insignia != "") then {["GetInsigniaTexture", [_insignia]] call MPSF_fnc_dynamicGroups} else {["GetInsigniaTexture", [_groupPicture]] call MPSF_fnc_dynamicGroups};
	...
	case "FillGroupsListbox" : {
		...
		tvClear _listbox;
		{
			...
			private _groupPicture 	= _x getVariable [VAR_GROUP_INSIGNIA, ["LoadRandomInsignia"] call MPSF_fnc_dynamicGroups];
			...
			private _wasKickedFrom	= ["WasPlayerKickedFrom", [_x, player]] call MPSF_fnc_dynamicGroups;
			private _hasInvite		= ["HasInvite", [_x, player]] call MPSF_fnc_dynamicGroups;
			...
			_listbox tvSetPicture [[_parent], ["GetInsigniaTexture", [_groupPicture]] call MPSF_fnc_dynamicGroups];
			if (_locked) then {
	...
	case "FillPlayersListbox" : {
		...
		lnbClear _listbox;
		{
			...
			private _texture 			= if (_isGroupLeader && ["IsGroupRegistered", [group _x]] call MPSF_fnc_dynamicGroups) then { _textureRank } else { _textureIcon };
			private _sameGroup			= group player == group _x;
			private _isDead				= !alive _x;
			private _isIncapacitated	= alive _x && _x getVariable ["BIS_revive_incapacitated", false];
			private _color = switch (true) do {
	...
	case "OnGroupsTreeSelChanged" : {
		...
		private _hasGroup = ["PlayerHasGroup", [player]] call MPSF_fnc_dynamicGroups;
		...
		if (_unsorted) exitWith {
			...
			if !(_hasGroup) then {
				...
				["setCtrlAction",["RscControlAmrelTabletSquadGroupButton","ButtonClick",{
					["OnCreateGroupButtonClick",[]] call MPSF_fnc_squadManager;
					["RefreshSquadUI"] call MPSF_fnc_squadManager;
				}]] call MPSF_fnc_itemAmrelTablet;
			};
		};

		private _group  = ["GetGroupByName", [_data, side group player]] call MPSF_fnc_dynamicGroups;
		...
		if !(isNull _group) then {
			...
			private _wasKickedFrom = ["WasPlayerKickedFrom", [_group, player]] call MPSF_fnc_dynamicGroups;
			private _isInGroup = _group == group player;
			private _hasInvite = ["HasInvite", [_group, player]] call MPSF_fnc_dynamicGroups;
			...
			switch (true) do {
				...
				case (_hasGroup && _isInGroup) : {
					...
					["setCtrlEnable",["RscControlAmrelTabletSquadGroupButton",true]] call MPSF_fnc_itemAmrelTablet;
					["setCtrlAction",["RscControlAmrelTabletSquadGroupButton","ButtonClick",{
						["OnLeaveGroupButtonClick",_this] call MPSF_fnc_squadManager;
						["RefreshSquadUI"] call MPSF_fnc_squadManager;
					}]] call MPSF_fnc_itemAmrelTablet;
				};
				case (_hasGroup);
				...
				default {
					...
					["setCtrlEnable",["RscControlAmrelTabletSquadGroupButton",true]] call MPSF_fnc_itemAmrelTablet;
					["setCtrlAction",["RscControlAmrelTabletSquadGroupButton","ButtonClick",{
						["OnJoinButtonClick",_this] call MPSF_fnc_squadManager;
						["RefreshSquadUI"] call MPSF_fnc_squadManager;
					}]] call MPSF_fnc_itemAmrelTablet;
				};
			};
			...
			private _hasInvite = ["HasInvite", [group player, _player]] call MPSF_fnc_dynamicGroups;
			private _sameGroup = group player == group _player;
			private _noGroup = !(["PlayerHasGroup", [player]] call MPSF_fnc_dynamicGroups);
			...
			switch (true) do {
				case (_isLeader && {_sameGroup} && {player != _player}) : {
					...
					["setCtrlEnable",["RscControlAmrelTabletSquadGroupButton",true]] call MPSF_fnc_itemAmrelTablet;
					["setCtrlAction",["RscControlAmrelTabletSquadGroupButton","ButtonClick",{
						["OnKickPlayerButtonClick",_this] call MPSF_fnc_squadManager;
						["RefreshSquadUI"] call MPSF_fnc_squadManager;
					}]] call MPSF_fnc_itemAmrelTablet;
				};
				case !(_hasGroup) : {
				...
				default {
					...
					["setCtrlEnable",["RscControlAmrelTabletSquadGroupButton",true]] call MPSF_fnc_itemAmrelTablet;
					["setCtrlAction",["RscControlAmrelTabletSquadGroupButton","ButtonClick",{
						["OnInvitePlayerButtonClick",_this] call MPSF_fnc_squadManager;
						["RefreshSquadUI"] call MPSF_fnc_squadManager;
					}]] call MPSF_fnc_itemAmrelTablet;
				};
			};
	...
	case "OnPlayerListSelChanged" : {
		...
		if !(["PlayerHasGroup", [player]] call MPSF_fnc_dynamicGroups) exitWith {
		...
		private _hasInvite = ["HasInvite", [group player, _clickedPlayer]] call MPSF_fnc_dynamicGroups;
	...
	case "OnJoinButtonClick" : {
		...
		private _group = ["GetGroupByName", [_groupUniqueId, side group player]] call MPSF_fnc_dynamicGroups;
		...
		if !(isNull _group) then {
			...
			if ((!_groupIsPrivate || {_hasInvite}) && {!_isFull}) then {
				if !(["PlayerHasGroup", [player]] call MPSF_fnc_dynamicGroups) then {
					["SendClientMessage", ["AddGroupMember", [_group, player]]] call MPSF_fnc_dynamicGroups;
				} else {
					["SendClientMessage", ["SwitchGroup", [_group, player]]] call MPSF_fnc_dynamicGroups;
				};

				if (_hasInvite) then {
					["RemoveInvite", [_group, player]] call MPSF_fnc_dynamicGroups;
				};

				// Event
				["OnGroupJoin", [_group, _groupLeader, player]] call MPSF_fnc_dynamicGroups;
			};
		};
	};
	case "OnCreateGroupButtonClick" : {
		...
		if (player == leader group player) then {
			["SendClientMessage", ["RegisterGroup", [group player, player]]] call MPSF_fnc_dynamicGroups;
		} else {
			["SendClientMessage", ["CreateGroupAndRegister", [player]]] call MPSF_fnc_dynamicGroups;
		};
	};
	case "OnLeaveGroupButtonClick" : {
		// Request server to make player leave the current group
		["SendClientMessage", ["RemoveGroupMember", [group player, player]]] call MPSF_fnc_dynamicGroups;
		// Event
		["OnGroupLeave", [group player, leader group player, player]] call MPSF_fnc_dynamicGroups;
	};
	case "OnKickPlayerButtonClick" : {
		...
		if (!isNull _group && {!isNull _groupLeader} && {!isNull _kickedPlayer} && {_groupLeader == player}) then {
			// Request server to make player leave the current group
			["SendClientMessage", ["KickPlayer", [_group, _groupLeader, _kickedPlayer]]] call MPSF_fnc_dynamicGroups;
			// Event
			["OnKicked", [_group, _kickedPlayer, _groupLeader]] call MPSF_fnc_dynamicGroups;
		};
	};
	case "OnInvitePlayerButtonClick" : {
		...
		if !(isMultiplayer) exitWith {
			["SwitchGroup",[_group,_clickedPlayer]] call MPSF_fnc_dynamicGroups;
		};

		// Add invite
		if !(["HasInvite", [_group, _clickedPlayer]] call MPSF_fnc_dynamicGroups) then {
			["AddInvite", [_group, player, _clickedPlayer]] call MPSF_fnc_dynamicGroups;
		};
	};
	case "OnPrivateStateChanged" : {
		_params params [["_checkboxPrivate",controlNull],["_isChecked",0,[0]]];
		["SendClientMessage", ["SetPrivateState", [group player,_isChecked == 1]]] call MPSF_fnc_dynamicGroups;
		["RefreshSquadUI"] call MPSF_fnc_squadManager;
	};
	case "SetGroupName" : {
	...
	case "OnGroupNameKillFocus" : {
		...
		private _groupsOfSide = ["GetAllGroupsOfSide", [side group player]] call MPSF_fnc_dynamicGroups;
		...
		if !(_nameTaken) then {
			...
			["SendClientMessage", ["SetName", [group player, _groupName]]] call MPSF_fnc_dynamicGroups;
...
/* Common */
	case "getUnregisteredPlayers" : {
		...
		{
			if (isPlayer _x && {side group _x == _side} && {!(["IsGroupRegistered", [group _x]] call MPSF_fnc_dynamicGroups)}) then {
...
/* Init */
	case "postInit" : {
		...
		if (isServer) then {
			//["Initialize", [true]] call MPSF_fnc_dynamicGroups;
			["Initialise", [true]] call MPSF_fnc_dynamicGroups;
		};

		if (hasInterface) then {
			[] spawn {
				waitUntil{ !(isNull player) };
				//["InitializePlayer",[player, true]] call MPSF_fnc_dynamicGroups;
				["InitialisePlayer",[player,false]] call MPSF_fnc_dynamicGroups;
				["RemoveKeyEvents",[displayNull]] spawn {sleep 1; _this call MPSF_fnc_dynamicGroups};
