...
["PO4_introScript_onComplete_EH","onIntroComplete",{
	...
	private _respawn = (["getRespawnPositions",[player]] call MPSF_fnc_respawnMP) param [0,[player,"Base",false]];
	[
		[
			[toUpper getText(configfile >> "CfgWorlds" >> worldName >> "description"), "align = 'center' shadow = '1' size = '1' font='PuristaBold'", "#ffc600"],
...
missionNamespace setVariable ["PO4_INTRO_SCRIPT",[] spawn {
	...
	if (side player isEqualTo west) then {
		playMusic ["vn_drafted",2];
	} else {
		playMusic ["vn_another_life",18];
	};
