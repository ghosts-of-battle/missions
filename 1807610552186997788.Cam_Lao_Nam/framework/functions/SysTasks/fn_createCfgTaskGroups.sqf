...

{
	_x params [
		...
		,["_revealPlayers",false,[false]]
		,["_attackNearestPlayer",false,[false]]
		,["_canCapture",false,[false]]
		,["_canQRF",false,[false]]
	];
	...
	if (_probability && _minPlayers <= count (allPlayers - entities "HeadlessClient_F") || !(isMultiplayer)) then {
		...
		if !(isNull (missionNamespace getVariable [_groupUID,objNull])) then {
			...
			if (count _vehicleTypes > 0) then {
				...
				if (count _vehicleTypes > 0) then {
					([_spawnPos,_factionID,selectRandom _vehicleTypes,_angle,if (count _crewTypes > 0) then {_crewTypes} else {_createCrew}] call MPSF_fnc_createVehicle) params [
			...
			if (count _groupTypes > 0) then {
				...
				if !(isNull _groupID) then {
					...
					switch (true) do {
						...
						case (_attackNearestPlayer && side _groupID isEqualTo civilian) : { [_groupID,_spawnPos,_radius] spawn MPSF_fnc_setGroupSIED; };
					...
					if (count _conversations > 0 && count _groupUnits > 0) then {
						...
						private _convTarget = _groupUnits param [0,objNull,[objNull]];
						if !(isNull _convTarget) then {
							[_convTarget,_conversationID,[_taskID]] call (missionNamespace getVariable ["MPSF_fnc_addConversation",{}]);
						};
					};

					if (_canQRF) then {
						[_groupID] call MPSF_fnc_setGroupQRF;
					};

					if (_canCapture) then {
						[_groupID] call MPSF_fnc_setGroupSurrender;
					};

					if (_dropIntel) then {
						{
							[_x,[_taskID,_cfgTaskID]] spawn (missionNamespace getVariable ["MPSF_fnc_onKilledDropIntel",{}]);
						} forEach _groupUnits;
					};

					if (_revealPlayers) then {
						{_groupID reveal [_x,3]} forEach allPlayers;
					};
			...
			if (count _objectTypes > 0) then {
				private _object = [selectRandom _objectTypes,_spawnPos,_angle,true] call MPSF_fnc_createObject;
				if !(isNull _object) then {
					...
					if (_canCapture) then {
						[_object] call MPSF_fnc_actionObjectHideDestroy;
					};

					[[_object],[_taskID],true] call MPSF_fnc_eventOnObjectDamagedC4;
				};
			};
		};
	};
...
//hintSilent ((_createdTargets + _createdUnits) joinString "\n");

[(_createdTargets + _createdUnits),[_taskID]] call MPSF_fnc_eventOnKilledTargets;
if ((["MPSF_Param_AIeasyKill",0] call BIS_fnc_getParamValue) isEqualTo 1) then {
	[_createdUnits] call MPSF_fnc_eventOnHandleDatamageMultiplier;
};
