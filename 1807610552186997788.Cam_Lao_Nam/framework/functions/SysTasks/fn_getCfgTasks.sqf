...
// Task Objects
	...
	case "SupplyTypes" : { ["CfgMissionTasks",_dataType] call MPSF_fnc_getCfgDataArray; };
// Tasks
	...
	case "getTasksByID" : {
		private _taskTypeIDs = _params apply { if (_x isEqualType "") then { _paramsText append (["taskTypeIDs",[_x]] call MPSF_fnc_getCfgTasks); -1 } else { _x }; };
		_taskTypeIDs = _taskTypeIDs select {_x > 0};
		(["taskIDs"] call MPSF_fnc_getCfgTasks) select {(["CfgMissionTasks",_x,"typeID"] call MPSF_fnc_getCfgDataNumber) in _taskTypeIDs};
	};
...
// Task Configuration
	case "isParent" : {
		...
		isClass (["CfgMissionTasks",_taskID,"ChildTasks"] call BIS_fnc_getCfg);
	};
	case "hasMarkers" : {
		_params params [["_taskID","",[""]]];
...
// Task Configuration: Conditions
	case "conditionAreas" : {
		_params params [["_taskID","",[""]]];
		["CfgMissionTasks",_taskID,"Conditions","areas"] call MPSF_fnc_getCfgDataArray;
	};
	case "conditionVehicleTypes" : {
		_params params [["_taskID","",[""]]];
		["CfgMissionTasks",_taskID,"Conditions","vehicleTypes"] call MPSF_fnc_getCfgDataArray;
	};
	case "conditionZoneTypes" : {
		_params params [["_taskID","",[""]]];
		["CfgMissionTasks",_taskID,"Conditions","zoneTypes"] call MPSF_fnc_getCfgDataArray;
	};
	case "conditionLimit" : {
		_params params [["_taskID","",[""]]];
		["CfgMissionTasks",_taskID,"Conditions","limit"] call MPSF_fnc_getCfgDataNumber;
	};
	case "conditionTimeout" : {
		_params params [["_taskID","",[""]]];
		["CfgMissionTasks",_taskID,"Conditions","timeout"] call MPSF_fnc_getCfgDataNumber;
	};
	case "conditionChat" : {
		_params params [["_taskID","",[""]]];
		["CfgMissionTasks",_taskID,"Conditions","sideChat"] call MPSF_fnc_getCfgDataText;
	};
	...
	case "Compositions" : {
		...
		private _compositions = [];
		{
			...
			_composition resize 11;
			{
				...
				,"distance"
				,"direction"
				,"radius" // TODO: Make it into an ARRAY
				,"angle"
				,"faction"
				,"groupTypes"
				,"targets"
			...
			private _intel = ["CfgMissionTasks",_taskID,_dataType,_compID,"intel"] call MPSF_fnc_getCfgDataBool;
			if !(_intel) then {
				_intel = ["CfgMissionTasks",_taskID,_dataType,_compID,"downloadIntel"] call MPSF_fnc_getCfgDataBool
			};
			_composition set [10,_intel];
...
// Task Configuration: Deployables
	...
	case "Groups" : {
		...
		private _groupArray = [];
		{
			...
			private _taskData = [
				...
				,["patrolAirspace","BOOL"]
				,["revealPlayers","BOOL"]
				,["attackNearestPlayer","BOOL"]
				,["canCapture","BOOL"]
				,["callQRF","BOOL"]
			];
			for "_i" from 1 to (count _taskData) do {
				(_taskData select (_i - 1)) params [["_varName",""],["_varType","ANY"]];
				_groupData set [_i,switch (_varType) do {
					case ("ARRAY") : {["CfgMissionTasks",_taskID,_dataType,_groupID,_varName] call MPSF_fnc_getCfgDataArray};
					case ("BOOL") : {["CfgMissionTasks",_taskID,_dataType,_groupID,_varName] call MPSF_fnc_getCfgDataBool};
					case ("NUMBER") : {["CfgMissionTasks",_taskID,_dataType,_groupID,_varName] call MPSF_fnc_getCfgDataNumber};
					case ("STRING") : {["CfgMissionTasks",_taskID,_dataType,_groupID,_varName] call MPSF_fnc_getCfgDataText};
					default { ["CfgMissionTasks",_taskID,_dataType,_groupID,_varName] call MPSF_fnc_getCfgData };
				}];
			};
...
// Task Data
	...
	case "childTasks" : {
		...
		private _childTaskIDs = ["CfgMissionTasks",_taskID,"ChildTasks",_dataType] call BIS_fnc_getCfgDataArray;
	...
	case "Objectives" : {
		...
		private _conditions = [];
		{
			...
			if (isClass(_cfg)) then {
				...
				private _nextTask = ["CfgMissionTasks",_taskID,"Objective",_objective,"nextTask"] call MPSF_fnc_getCfgDataArray;
				private _nextTasks = ["CfgMissionTasks",_taskID,"Objective",_objective,"nextTasks"] call MPSF_fnc_getCfgDataArray;
				private _function = ["CfgMissionTasks",_taskID,"Objective",_objective,"function"] call MPSF_fnc_getCfgDataText;
				private _endMission = ["CfgMissionTasks",_taskID,"Objective",_objective,"endMission"] call MPSF_fnc_getCfgDataBool;
