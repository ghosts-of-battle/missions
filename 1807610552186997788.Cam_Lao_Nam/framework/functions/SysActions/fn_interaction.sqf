...
switch (_mode) do {
	case "addAction" : {
		...
		if ((_object getVariable [_actionID,-1]) >= 0) then {
			_object removeAction (_object getVariable [_actionID,-1]);
			//[_actionID,_object,_respawn] call MPSF_fnc_eventOnRemoveAction;
		};
	...
	case "respawn" : {
		{ _x set [1,player]; _x set [9,true]; _x call MPSF_fnc_addAction; } forEach (missionNamespace getVariable ["MPSF_InterAction_playerActions",[]]);
	};
	case "init" : {
		if (hasInterface && !(missionNamespace getVariable ["MPSF_fnc_interaction_init",false])) then {
			missionNamespace setVariable ["MPSF_fnc_interaction_init",true];
