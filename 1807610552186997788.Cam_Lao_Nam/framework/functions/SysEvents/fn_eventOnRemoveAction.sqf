...
*/
params [["_actionID","",[""]],["_object",objNull,[objNull]],["_respawn",false,[false]]];

if (isServer) then {
	private _globalActions = missionNamespace getVariable ["MPSF_InterAction_globalActions",[]];
	{
		if ((_x select 0) isEqualTo _actionID && !_respawn) exitWith { _globalActions deleteAt _forEachIndex; };
...
if (hasInterface) then {
	...
	{
		if ((_x select 0) isEqualTo _actionID && !_respawn) exitWith { _localActions deleteAt _forEachIndex; };
	...
	{
		if ((_x select 0) isEqualTo _actionID && !_respawn) exitWith { _playerActions deleteAt _forEachIndex; };
