...
class VN_Task_Clear_Radar {
	...
	areaSize[] = {1000,1000};
	positionSearchRadius = 90000;
	positionSearchTypes[] = {"Radar"};
	positionIsWater = 0;

	class TaskDetails {
		title = "%7 occupied Radar";
		description[] = {
			"<t>Ref: %1</t> | <t>Date: %2<br/>AO: %3 %4 near %5</t>"
			,"<t size='1.1' color='#FFC600'>Brief:</t> <t>INTEL report identifies a %7 squads are at a radar station near %5 and appears to be setting up an outpost.</t>"
			,"<t size='1.1' color='#FFC600'>Action:</t> <t>%6 forces will move to the area to conduct a sweep in order to locate and attack any %7 squads in that area.</t>"
			,"<t size='1.1' color='#FFC600'>Enemy:</t> <t>%7 forces are minimal with a likelyhood of only one small squad operating in that area with small arms and possibly an additional light technical. They are likely to call in support when engaged so move quickly.</t>"
		};
		iconType = "default";
