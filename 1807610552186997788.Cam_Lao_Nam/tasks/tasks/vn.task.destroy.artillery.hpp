...
class Task_Destroy_Artillery {
	...
	class TaskDetails {
		...
		description[] = {
			...
			,"<t size='1.1' color='#FFC600'>Brief:</t> <t>INTEL indicates %7 have shelled a village after the locals assisted %6 near %5. It is unknown if this is small rockets or something larger however poses an immediate threat and must be eliminated.</t>"
			,"<t size='1.1' color='#FFC600'>Action:</t> <t>%6 forces will move to the area to conduct a sweep in order to locate and destroy the target rocket device or artillery devices.</t>"
			,"<t size='1.1' color='#FFC600'>Enemy:</t> <t>%7 forces are minimal with a likelyhood of only one small rocket squad operating in that area with small arms and possibly an additional light technical.</t>"
	...
	class Groups {
		class Target_Veh_AT {
			...
			vehicleTypes[] = {"Armour_Art"};
			createCrew = 1;
			radius[] = {80,120};
			distance[] = {20,40};
			direction[] = {0,180};
		};
		class Target_Veh_AT_2: Target_Veh_AT {
			probability = 0.7;
			minPlayers = 8;
			distance[] = {20,40};
			direction[] = {180,360};
