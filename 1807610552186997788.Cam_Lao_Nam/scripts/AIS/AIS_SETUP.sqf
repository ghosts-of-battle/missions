...
AIS_REVIVE_INIT_UNITS 		= 	"allUnitsOpfor"; // Auto-Init a group of units: "allPlayers" , "allPlayables" , "allUnits", "allUnitsBLUFOR", "allUnitsOPFOR", "allUnitsINDFOR", "allUnitsCIVILIAN"
...
AIS_REVIVETIME 				= 	15;				// Basic revive time in seconds. The real revive time depends on the real damage of the unit. (can be less or more time from the basic value)
...
AIS_DISABLE_RESPAWN_BUTTON	=	0;				// Time in seconds while the respawn button is disabled (Esc Menu). Set to 0 to enable the respawn button everytime.
