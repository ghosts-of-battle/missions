...
{if(alive _x)then{if(_x getUnitTrait"RTO")then{if((side _x==side player)||(_x isKindOf r_PS))then{_S0 pushBack _x;};}else{
...
if(assignedVehicleRole _x in _rtoSeat)then{_s0 pushBack _x;};};};};};
}forEach allPlayers-[player];
if(count _s0<1)exitWith{};
private _s1=([_s0,[],{player distance _x},"ASCEND"]call BIS_fnc_sortBy)select 0;private _d0=player distance _s1;//_s1 undefined
if(_d0<=r_LRmax)then{
